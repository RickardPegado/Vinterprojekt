from selenium import webdriver
from selenium.webdriver.common.by import By
from login_data import LoginData
from login_data import ElemLocators
from login_data import items_desc_list
import pytest_check as check
import pytest


class TestUrl:
    url = "https://www.saucedemo.com/v1/index.html"


    @pytest.fixture
    def launch_driver(self):
        self.driver = webdriver.Chrome()
        self.driver.maximize_window()
        self.driver.implicitly_wait(8)
        yield
        self.driver.close()


    def test_item_details_standard_user(self, launch_driver):
        self.driver.get(self.url)
        assert self.driver.current_url == "https://www.saucedemo.com/v1/index.html"
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_username).send_keys(LoginData.user1)
        assert self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_username).get_attribute('value') == LoginData.user1
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_password).send_keys(LoginData.password)
        assert self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_password).get_attribute('value') == LoginData.password
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_login).click()
        product = self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_product).text
        assert product == 'Products'
        print("SUCCESS # LOGGED IN WITH USERNAME {username} and PASSWORD {password}".format(username=LoginData.user1, password=LoginData.password))

        inventory_container = self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_inventoryitem)

        correct_name_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_name')]
        correct_desc_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_desc')]
        correct_price_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_price')]
        correct_img_list = []
        elements = inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_img')
        
        for element in elements:
            src_attribute = element.get_attribute('src')
            if src_attribute is not None:
                correct_img_list.append(src_attribute)

        for x in range(6):

            self.driver.find_element(by=By.XPATH, value=(items_desc_list[x-1])).click()

            item_name = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_name").text
            item_description = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_desc").text
            item_price = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_price").text
            item_image = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_img").get_attribute("src")
                                                                                                                 
            assert item_name == correct_name_list[x-1]
            assert item_description == correct_desc_list[x-1]
            assert item_price == correct_price_list[x-1]
            assert item_image == correct_img_list[x-1]

            self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_back_button").click()
            assert product == 'Products'


    def test_item_details_problem_user(self, launch_driver):
        self.driver.get(self.url)
        assert self.driver.current_url == "https://www.saucedemo.com/v1/index.html"
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_username).send_keys(LoginData.user3)
        assert self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_username).get_attribute('value') == LoginData.user3
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_password).send_keys(LoginData.password)
        assert self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_password).get_attribute('value') == LoginData.password
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_login).click()
        product = self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_product).text
        assert product == 'Products'
        print("SUCCESS # LOGGED IN WITH USERNAME {username} and PASSWORD {password}".format(username=LoginData.user3, password=LoginData.password))

        inventory_container = self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_inventoryitem)

        correct_name_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_name')]
        correct_desc_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_desc')]
        correct_price_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_price')]
        correct_img_list = []
        elements = inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_img')
        
        for element in elements:
            src_attribute = element.get_attribute('src')
            if src_attribute is not None:
                correct_img_list.append(src_attribute)

        for index, item_xpath in enumerate(items_desc_list):

            self.driver.find_element(by=By.XPATH, value=item_xpath).click()

            item_name = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_name").text
            item_description = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_desc").text
            item_price = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_price").text
            item_image = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_img").get_attribute("src")
                                                                                                                 
            check.equal(item_name, correct_name_list[index])
            check.equal(item_description, correct_desc_list[index])
            check.equal(item_price, correct_price_list[index])
            check.equal(item_image, correct_img_list[index])

            self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_back_button").click()
            assert product == 'Products'


    def test_item_details_glitch_user(self, launch_driver):
        self.driver.get(self.url)
        assert self.driver.current_url == "https://www.saucedemo.com/v1/index.html"
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_username).send_keys(LoginData.user4)
        assert self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_username).get_attribute('value') == LoginData.user4
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_password).send_keys(LoginData.password)
        assert self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_password).get_attribute('value') == LoginData.password
        self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_login).click()
        product = self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_product).text
        assert product == 'Products'
        print("SUCCESS # LOGGED IN WITH USERNAME {username} and PASSWORD {password}".format(username=LoginData.user4, password=LoginData.password))

        inventory_container = self.driver.find_element(by=By.XPATH, value=ElemLocators.xpath_inventoryitem)

        correct_name_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_name')]
        correct_desc_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_desc')]
        correct_price_list = [element.text for element in inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_price')]
        correct_img_list = []
        elements = inventory_container.find_elements(by=By.CLASS_NAME, value='inventory_item_img')
        
        for element in elements:
            src_attribute = element.get_attribute('src')
            if src_attribute is not None:
                correct_img_list.append(src_attribute)

        for x in range(6):

            self.driver.find_element(by=By.XPATH, value=(items_desc_list[x-1])).click()

            item_name = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_name").text
            item_description = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_desc").text
            item_price = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_price").text
            item_image = self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_img").get_attribute("src")
                                                                                                                 
            assert item_name == correct_name_list[x-1]
            assert item_description == correct_desc_list[x-1]
            assert item_price == correct_price_list[x-1]
            assert item_image == correct_img_list[x-1]

            self.driver.find_element(by=By.CLASS_NAME, value="inventory_details_back_button").click()
            assert product == 'Products'
